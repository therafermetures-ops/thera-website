...
export default function AdminParametres() {
  ...
  useEffect(() => {
    fetch('/api/admin/config?categorie=contact')
      .then(r => r.json())
  ...
  return (
    <div className="p-8 max-w-2xl">
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">Informations de contact et horaires</p>
      </div>

      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-5">
        {configs.map(c => (
          <div key={c.cle}>
            <label className="block text-sm font-semibold text-gray-700 mb-2">{LABELS[c.cle] || c.cle}</label>
            <input
              type="text"
              value={c.valeur || ''}
