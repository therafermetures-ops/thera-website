...
async function getStats() {
  ...
  const [{ count: total }, { count: publiees }, { count: brouillons }] = await Promise.all([
    db.from('actualites').select('*', { count: 'exact', head: true }),
    db.from('actualites').select('*', { count: 'exact', head: true }).eq('statut', 'publiee'),
    db.from('actualites').select('*', { count: 'exact', head: true }).eq('statut', 'brouillon'),
  ])
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
        {[
          { label: 'Actualités totales', value: total ?? 0, color: 'text-blue-700', border: 'border-blue-100' },
          { label: 'Publiées', value: publiees ?? 0, color: 'text-green-700', border: 'border-green-100' },
          { label: 'Brouillons', value: brouillons ?? 0, color: 'text-yellow-700', border: 'border-yellow-100' },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-10">
        <Link href="/admin/actualites/nouveau"
          className="bg-orange-500 hover:bg-orange-600 text-white rounded-2xl p-6 flex items-center gap-4 transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          ...
          <Link href="/admin/actualites" className="text-orange-500 hover:text-orange-600 text-sm font-semibold">Voir tout →</Link>
        </div>
          ...
          {recentes && recentes.length > 0 ? recentes.map((a: { id: string; titre: string; statut: string; date_publiee: string }) => (
            <div key={a.id} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition-colors">
              ...
              <div className="flex items-center gap-3">
                <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${a.statut === 'publiee' ? 'bg-green-100 text-green-700' : a.statut === 'brouillon' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500'}`}>
                  {a.statut === 'publiee' ? 'Publiée' : a.statut === 'brouillon' ? 'Brouillon' : 'Archive'}
                </span>
                <Link href={`/admin/actualites/${a.id}`} className="text-gray-400 hover:text-orange-500 text-sm">Modifier →</Link>
              </div>
            </div>
          )) : (
            <div className="px-6 py-10 text-center text-gray-400">
              <p className="text-lg mb-2">Aucune actualité</p>
              <Link href="/admin/actualites/nouveau" className="text-orange-500 hover:underline text-sm">Créer la première →</Link>
            </div>
          )}
        </div>
      </div>
